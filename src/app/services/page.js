...
const Cards = () => {
 ...
 // Handle button click and redirect to the /contact page with full title as a query parameter
 const handleBookNow = (title) => {
  ...
  return (
      <div className='m-2'>
      <h3 className="text-4xl font-headerfont font-semibold mb-8 text-center justify-center items-center bg-gray-400 py-2">Our Trending Services</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, index) => (
            <div key={index} className="bg-orange-50 rounded-lg shadow-lg overflow-hidden z-10">
              <img src={service.image} alt={service.title} className="w-full h-60 object-cover" />
              <div className="p-4">
                <h4 className="text-lg font-semibold justify-center text-center">{service.title}</h4>
                <div className='flex justify-between pt-2'>
                  <div className='ml-8 pt-3'>
                  {service.originalPrice && (
                  <span className="text-lg font-medium line-through text-gray-500 mr-2">{service.originalPrice}</span>
                )}
                {service.discountedPrice && (
                  <span className="text-lg font-medium text-black">{service.discountedPrice}</span>
                )}
                </div>
                  <button
                onClick={() => handleBookNow(service.title)} // Pass the full service title
